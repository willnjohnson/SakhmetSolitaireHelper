...
// Applies highlighting based on the current move suggestion.
function applyMoveHighlight(suggestion) {
    clearAllHighlights(); // Always clear previous highlights first

    // If the suggestion is "Collect Winnings", do NOT apply any highlights.
    ...
    if(suggestion.includes('Draw')) {
        ...
        if(drawPilePyramid) {
            applyHighlight(drawPilePyramid, 'cyan'); // Highlight main deck pyramid
    ...
    // Handle "Flip top card of T#"
    if(suggestion.includes('Flip top card')) {
    ...
    if(sourceResult.isChosen) {
        // console.log('applyMoveHighlight: Source card is already chosen. Attempting to highlight destination.');
        ...
        // console.log('applyMoveHighlight: Source card is not chosen. Highlighting source.');
...
(function() {
    ...
    setTimeout(() => {
        ...
        // MutationObserver for dynamic highlighting
        const observer = new MutationObserver((mutations) => {
            let shouldUpdateHighlight = false;
            mutations.forEach((mutation) => {
                if(mutation.type === 'attributes' && mutation.attributeName === 'src') {
                    const img = mutation.target;
                    // Check if a card image's src changed to _chosen.gif or from _chosen.gif
                    if(img.src.includes('_chosen.gif') || (mutation.oldValue && mutation.oldValue.includes('_chosen.gif'))) {
                        shouldUpdateHighlight = true;
                    }
                }
            });

            if(shouldUpdateHighlight && currentSuggestion) {
                // console.log('MutationObserver: Detected card selection change, re-applying highlight for:', currentSuggestion);
                applyMoveHighlight(currentSuggestion);
            }
        });
