...
function applyMoveHighlight(suggestion) {
    ...
    if (suggestion.includes('Collect Winnings')) return;
    ...
    if (suggestion.includes('Draw')) {
        ...
        if (drawPilePyramid) applyHighlight(drawPilePyramid, 'cyan');
        else if (drawPileRoundEnd) applyHighlight(drawPileRoundEnd, 'cyan');
        return;
    }
    ...
    if (suggestion.includes('Flip top card')) {
        ...
        if (tableauMatch) {
            ...
            if (columnTd) {
                ...
                if (faceDownCards.length > 0) applyHighlight(faceDownCards[faceDownCards.length - 1], 'blue');
                else console.warn(`applyMoveHighlight: Could not find face-down card to flip in T${colIndex}`);
            }
        }
        return;
    }
    ...
    if (sourceResult.isChosen) {
        ...
        if (destElement) applyHighlight(destElement, 'lime');
        else console.warn("applyMoveHighlight: Could not find destination element for:", suggestion);
    } else {
...
(function () {
    ...
    setTimeout(() => {
        ...
        const observer = new MutationObserver(mutations => {
            ...
            if (shouldUpdate && currentSuggestion) applyMoveHighlight(currentSuggestion);
        });
